...
本合同适用于 `docs/**/content-v0.1.md` 至 `content-v0.6.md` 的全部内容文件。每个内容文件中的表格、键和值是该版本的**覆盖定义**；未在表中覆盖的字段按本合同和对应领域的 `model.md`、`workflow.md`、`use-cases.md` 解释。若冲突，优先级为：内容文件明确条目 > 本合同 > 领域模型/流程 > 总设计。
...
实现某条内容前，测试最少覆盖：内容存在/关闭、所有前置条件、资源不足、配额/冷却、状态冲突、成功、重复 operation、不同输入冲突、随机重放（如有）、持久化异常回滚、文本/按钮同一用例，以及该域列出的特有失败路径。内容文件要增加新键时，必须同时更新 `extensions/content/content-v*.md` 的包清单与引用校验。
