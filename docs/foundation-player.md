...
> **兼容概览，非权威。** 开发时先读[开发文档总入口](development-guide.md)，再进入[角色域](foundation/player/)；当前状态以[当前开发状态](current-status.md)为准。
...
系统创建平台身份映射、内部角色 ID、创建时间、场景偏好和最小状态。玩家可在
`开始修仙 <道号>` 时取一个最多 7 个字且不可重复的道号；不填写时保持未命名。
此时不发放大量灵石、装备或抽奖物品。
...
- `修仙改名 <道号>` 检查长度、敏感词和重复名；未命名角色首次改名不需要改名卡，之后需要 `item.token.rename_card`。
...
| `dao_name` | 0-7 个 Unicode 字符 | 用户可见道号；非空时全局唯一 |
| `nickname` | 平台字符串 | 内部身份记录，不作为用户展示名 |
