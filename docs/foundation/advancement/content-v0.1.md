...
- `skill.basic_attack`、六道途 v0.1 主动技能可升 1–3 级；按目标等级结算：0→1 消耗技能心得 1、灵石 20，1→2 消耗技能心得 1、灵石 40，2→3 消耗技能心得 2、灵石 80；效果基础值每级增加 300 bp；资源不足时事务零变化。
- 运行时命令为 `神通预览`、`我的神通`、`参悟神通 <技能>`（别名 `升级神通`）。技能心得是角色绑定资源，独立于物品背包；参悟保存技能等级、效果和资格/境界/地点快照，当前只作为后续战斗构筑来源，不启动战斗运行时。
...
- 运行时命令为 `法器预览`、`强化法器 <木纹剑|棉袍>`、`重铸预览`、`重铸法器 <木纹剑|棉袍>`。
  唯一装备使用 `equipment_instances` 保存强化等级、耐久、词条和保底计数；祭炼/重铸流水、版本快照和
  operation ledger 必须在同一短事务内写入。旧堆叠库存中的唯一法器只在资源充足且操作实际执行时懒迁移，
  资源不足、身份无效或长时行动冲突都不得改变库存、实例或流水。
