...
|:--|:--|:--|:--|:--|
| `item.pill.soul_condense` | 突破丹，9 | 绑定 | 元婴突破必需；成功/失败均消耗 | 元婴炼丹、跨界声望商店 |
| `item.pill.soul_restore` | 心魔丹，9 | 绑定 | 仅 `heart_demon.purify` 消耗；降低疲劳/污染 | 心魔试炼、炼丹 |
| `item.soul_crystal` | 神魂材料，99 | 可交易 | 元婴突破、神魂技能、多人副本复活 | 跨界秘境 |
| `item.demon_core` | 魔界材料，99 | 可交易，首次获得绑定 24h | 元婴突破替代材料、魔修契约、高阶炼器 | 魔渊 |
...
`loot.demon.abyss.v0.3` 与 `loot.beast.hills.v0.3` 在会话开始时按地点、队伍、阵营和版本冻结；魔渊额外池按魔核 45、声望 30、契约线索 15、心魔 10 抽取，其中 `item.clue.demon_contract` 为绑定堆叠线索，心魔事件仍由后续切片接入。`demon_core`/`beast_blood` 的首次 24 小时绑定写入实例/批次，不允许通过拆分堆叠绕过。`boundary_spear` 与 `soul_robe` 只能由轮次奖励 operation 发放；轮次重复领奖返回原实例引用。
