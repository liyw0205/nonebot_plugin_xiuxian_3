...
`loot.demon.abyss.v0.3` 与 `loot.beast.hills.v0.3` 在会话开始时按地点、队伍、阵营和版本冻结；`demon_core`/`beast_blood` 的首次 24 小时绑定写入实例/批次，不允许通过拆分堆叠绕过。`boundary_spear` 与 `soul_robe` 只能由轮次奖励 operation 发放；轮次重复领奖返回原实例引用。
