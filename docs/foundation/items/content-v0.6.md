...
|:--|:--|:--|:--|:--|
| `item.tribulation_token` | 天劫凭证，9 | 绑定、不可交易 | 前往天劫台消耗 1；每次三阶段试炼另消耗 1；最终战不消耗 | 合道许可 1；三项道源任务各在当季 3/3 完成时发放 1 |
| `item.dao_fruit_fragment` | 道果材料，99 | 绑定、不可交易 | 合道前置 10；道源门行程 2；试炼奖励/终局配方 | `quest.dao_union` 许可奖励 12；身心劫奖励 |
| `item.ascension_certificate` | 资格凭证，1 | 绑定 | 创建终局战大厅时托管；胜利/留界结局消耗，失败/取消/大厅超时返还 | 三次试炼成功、道统授予 |
| `item.weapon.dao_origin` | 终局法器，唯一 | 永久绑定 | 道果进度获得 +1000 bp（受试炼上限）；不可拆解 | 终局首领唯一奖励 |
| `item.title.ascended` | 称号，唯一 | 永久绑定 | 飞升/留界展示；不提供货币或伤害 | 个人结局 operation |
| `item.tribulation_guard` | 天劫保护阵，1 | 绑定至下一试炼 | 失败时 `resource.tribulation_debt` 增量 -5，最低 0 | `recipe.tribulation.guard` |

天劫 token 在天劫台移动成功创建和试炼成功创建时分别消耗；准入失败、活动会话互斥或 operation 冲突时不扣除。完成三次试炼链至少需要 4 张天劫凭证（1 张移动、每次试炼 1 张）。道果碎片不能直接兑换灵石、世界功勋或飞升资格。终局战运行中超时按失败结算并返还托管凭证；大厅超时在下一次会话访问/操作时释放成员锁并返还凭证，不得静默消失。
