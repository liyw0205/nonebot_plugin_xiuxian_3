...
元婴 L1 后开放 `cultivate.soul_refinement`（`progression-0.4.1`）：每次持续 30 分钟，消耗体力 8、精力 5，业务日最多 2 次；按角色感悟快照获得基础修为 5,000，并增加神魂 50。首次使用将神魂上限提高到至少 300，神魂不超过上限；普通静修和调息规则不变。会话开始冻结境界、感悟、状态、修为参数和神魂收益，结算/过期恢复使用同一快照。

元婴突破后神魂初始化为 `100/300`。界隙秘境服务端胜利结算另为每名有效贡献成员增加神魂 50（上限至少 300，当前值仍受实际上限约束）；失败仍按界隙战斗规则扣减并进入疲劳。神魂淬炼可补充当前神魂，不能绕过元婴境界门槛。
