...
本文件遵守 [版本内容开发合同](../../content-development-contract.md)。`content_version=content-0.3`，`rule_version=events-0.3.2`。
...
运行时入口为 `三界赛季 [阵营功勋|多人副本贡献|宗门贡献] [赛季编号]` 与
`领取三界赛季奖励 <赛季编号>`。赛季结束后首次查询或领奖按需冻结匿名三榜；公开结果只含匿名别名、名次、积分和达成时间，不显示平台用户 ID。领奖 operation 可重放，另一个 operation 重复领取返回专用错误；超过 7 天窗口只记录过期结果。绑定神魂晶写入独立赛季绑定审计，并由摆摊、拍卖、求购事务统一拦截。

赛季进行中每次查询或冻结前都会把事件贡献、已结算多人副本贡献和宗门贡献投影到
`three_realms_season_score_events`；投影键为 `(season_id, board_key, player_id, source_operation_id)`，因此来源重放不会重复计分。冻结后只读取 `three_realms_rankings`，后续来源不会改写历史榜单。
