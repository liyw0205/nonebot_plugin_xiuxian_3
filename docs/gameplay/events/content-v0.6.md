...
| `task.dao_origin.guard` | 合道；完成守界防御任务 3 次 | +150 | +150 | 每赛季一次 |
| `task.dao_origin.build` | 合道；完成留界建设/领域维护任务 3 次 | +160 | +150 | 每赛季一次 |
| `task.dao_origin.teach` | 合道；完成师徒/道统传承任务 3 次 | +160 | +150 | 每赛季一次 |

三道源任务与三次天劫试炼合计提供 1000 道果进度和 1000 `resource.ascension_merit`，满足最终战可达性。任务完成时在同一 operation 写进度、功勋和 `event.dao_origin` 贡献；重复请求只返回原结果。职业终局作品引用 `recipe.dao.fruit_fragment`、`recipe.tribulation.guard` 或道途对应大师作品，按 `quest.dao_union` 快照选择一种。
