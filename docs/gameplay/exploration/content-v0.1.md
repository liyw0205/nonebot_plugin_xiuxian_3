...
探索开始时扣体力、固定角色/地点/装备/道途/随机池/次数；状态 `created -> running -> settled | expired`。只有 `created` 可取消，返还全部体力，不抽池。`running` 后不可取消；超过结束时间 24 小时由恢复任务结算。战斗遭遇由固定 roll 决定，战斗结果未结算前探索保持 `combat_pending`，不能二次抽探索奖励。
...
关闭时停止新会话/接取；已有 running/combat_pending 会话按原池结算或恢复任务标记 `expired` 并保留成本/原因。验收：前置/体力不足不扣费；随机与遭遇重放稳定；取消只在 created；战斗失败不重抽资源；每日上限和悬赏领取并发安全；文本/按钮共享同一用例。