...
|:--|:--|:--|:--|:--|
| `explore.cloud_mine` | 云铁矿区、筑基、采矿 2 或有效期内已接取的矿区悬赏 | 2 分钟 / 8 体力、2 精力 | `gather.cloud_mine.v0.2`：云铁 1–4 | 30% 矿兽战；每日 6 |
...
失败/过期不回收探索产出，奖励不发。当前运行时已接入 `explore.cloud_mine`、
`explore.cloud_boat_trial` 和 `explore.mist_grotto_2`；云舟风暴选择支持等待、支付、返航、超时默认等待，且在 QQ 官方与 OneBot V11 模拟适配器下验证了奖励快照、灵石/体力结算和 operation 幂等。悬赏轮次仍未接入。
