...
|:--|:--|:--|:--|
| `residence.town_room` | `mortal` 以上；租金 20 灵石/3 业务日 | 开放 1 个休整位、1 个灵田、1 个私人物资箱（10 格） | 逾期进入 `overdue`，不可开启新休整/地块，已有订单继续结算 |
...
|:--|:--|:--|:--|
| `service.gather_help` | 信誉 >=10；同地点；体力 3 | 为委托人完成 1 次教学采集，发布人锁定灵石 15 | 每日 3；采集失败仅退 1 体力，不收服务报酬 |
| `service.cook_meal` | 烹饪教学、精力 2、粗糙灵米 2 | 交付灵米饭 2；默认锁定灵石 20（发布时可指定 1–100） | 每日 5；沿用生产质量/失败快照 |
| `route.new_town_outskirts` | `mortal` 以上；青石镇出发；货物价值 <=100 灵石；体力 2 | 10 分钟后固定报酬 12 灵石、名望 +2 | 每日 3；`route.town.v0.1` 10% 延误，延误仅多 10 分钟不丢货 |

服务订单状态和报酬锁定按 `LivelihoodOrder`。发布时立即锁定报酬，发布人可用 `取消服务 订单号` 在 `published` 状态取消并全额解锁；承接后材料与精力进入锁定，成功交付支付承接者 9800 bp，平台回收 200 bp；失败或 24h 超时按快照退未消耗材料和 8000 bp 报酬，承接者信誉不变。

短途运输使用 `运输预览 [货物] [数量]`、`开始运输 [货物] [数量]` 和 `结算运输 [路线号]`。v0.1 的货物快照按固定单位货值计算：粗糙灵米 5、灵米饭 12、止血草 10、灵叶 18、木材 8、铁石 15、阵砂 20 灵石；总货值不得超过 100。开始时锁定货物并消耗体力 2，路线从青石镇前往近郊；基础 10 分钟，`route.town.v0.1` 固定随机池有 10% 概率额外延误 10 分钟。抵达后交付货物并发放 12 灵石和地方名望 +2，路线在 `in_transit` 期间不可取消；失败只发生在服务端恢复异常并全额返还货物、不支付报酬。
