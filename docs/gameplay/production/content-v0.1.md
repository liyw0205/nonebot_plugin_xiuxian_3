...
本文件遵守 [版本内容开发合同](../../content-development-contract.md)。`content_version=content-0.1`，基础配方使用 `rule_version=production-0.1.0`，本阶段新增护脉丹配方使用 `production-0.1.1`。首版开放炼丹、炼器、布阵各一条个人生产闭环；烹饪为生活子类，使用统一订单状态机但不要求主辅修。
...
| `recipe.pill.qi_guard` | `alchemy` 1、感气 | 灵叶 2、止血草 2 | 炉，6 精力，60 秒，耐久 -100 bp | `item.pill.qi_guard` 1（绑定） | 每日 3；失败按每类输入 50% 向下返还 |
...
| `recipe.pill.foundation_guard` | `alchemy` 2、聚气 | 灵叶 2、阵砂 1、止血草 1 | 炉，10 精力，180 秒，耐久 -100 bp | `item.pill.foundation_guard` 1（绑定） | 每日 2；失败按每类输入 50% 向下返还 |
...
当前运行时切片开放 `recipe.pill.healing_low`、`recipe.pill.focus_low`、`recipe.pill.qi_guard`、`recipe.pill.foundation_draft`、`recipe.pill.foundation_guard`、`recipe.weapon.wood_sword` 和
`recipe.array.gathering_basic` 的个人订单；命令为 `生产预览 <配方>`、`开始生产 <配方>`、
`领取生产` 和 `恢复生产`。炼丹教学完成且选择炼丹辅修的感气角色可以制作低阶疗伤丹和焦点丹；
感气炼丹辅修角色可以制作绑定的聚气护脉丹，聚气炼丹辅修角色可以制作绑定的筑基丹和筑基护脉丹；
这些成品分别作为聚气/筑基突破的正式个人生产来源。筑基护脉丹另可由 v0.2 宗门商店按固定贡献兑换，库存来自宗主/副宗主消耗公共仓库原料和灵石的宗门补给；个人绑定丹药不能直接捐入仓库。
