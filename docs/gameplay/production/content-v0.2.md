...
本文件遵守 [版本内容开发合同](../../content-development-contract.md)。`content_version=content-0.2`，默认 `rule_version=production-0.2.0`；凝核丹前置修订为 `production-0.2.1`。金丹生产质量阈值提高到 6000；失败返还每类可返材料 60%，工具额外 -150 bp 耐久。
...
| `recipe.pill.core_condense` | `alchemy` 4、筑基起（含金丹） | 灵叶 5、云铁 2、洞天材料 2 | 炉，10 精力，180 秒 | `item.pill.core_condense` 1 | 每日 2；绑定；本条使用 `production-0.2.1` |
...
凝核丹是金丹突破必需的绑定材料；筑基炼丹辅修者可提前制作，金丹角色原有生产入口不变。
