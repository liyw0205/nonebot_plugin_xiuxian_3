...
`SectWarFederationSnapshot` / `SectWarFederationResult`：跨服宗门战前置数据。前者保存本地宗门最多 15 名实际出战成员的不可变快照和 shard 键，后者保存带来源 operation 的远端轮次分数/胜负。两者只写联邦审计表，不触发跨服匹配、身份合并、奖励发放或本地资产变化；同一结果重放幂等，字段冲突拒绝。
