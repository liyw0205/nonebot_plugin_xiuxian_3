...
界隙队伍只能在 `cave.boundary_realm` 创建。开始前服务端校验所有成员元婴 L1、`story.mainline.three_realms` 证据、地点、30 体力和队长 1 枚 `item.soul_crystal`；任一失败整体拒绝且不扣资源。成功后保存阵营/盟约/污染/血脉/跨界惩罚和版本快照，自动战斗使用 `enemy.boundary_watcher`。失败仅扣神魂并进入疲劳，不掉永久装备；奖励按成员独立写入且不可重复发放。
