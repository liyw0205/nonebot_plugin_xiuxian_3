...
当前运行时开放 `forming -> ready -> disbanded/expired` 的双人探索队伍、普通 4–5 人副本队伍、三人竞技队伍、界隙/魔渊/万兽 2–5 人队伍状态机，
以及独立的 `party_battle` 自动 PVE 会话；队伍自身不伪装成单人战斗会话。邀请和全员确认窗口为 5 分钟，
队伍创建时保存地点和分配规则。普通多人副本要求至少 4 名、最多 5 名同地点成员，并在 `xuantian.outskirts` 或 `cave.mist_grotto` 选择普通敌人；
界隙队伍只能在 `cave.boundary_realm` 创建，开始前要求所有成员元婴 L1、三界主线证据、
...
普通多人副本开始事务会先验证 4–5 名成员全部确认、地点一致和普通敌人映射，再写入独立 `party_battle_sessions`，冻结属性/装备/技能快照并锁定成员资产；重复创建、确认、启动和结算只返回原结果，不重复建会话或发奖。

个人竞技场另有 `arena.three_realms` 快照流程：发布时原子校验元婴 L1、三界竞技许可和阵营，冻结阵营/盟约/污染/血脉；30 分钟后允许同阵营或跨阵营匹配。挑战由服务端选择行动，结果与战术环境写入独立回放和 operation ledger，重放不重复计分或投影；阵营声望、物品、灵石、修为和装备保持不变。

跨服宗门战当前只开放前置数据流程：宗主/副宗主可为已报名轮次冻结本地最多 15 人 roster，联邦侧导入带来源 operation 的外部结果并做幂等冲突校验。流程不创建跨服对局、不合并 QQ/OneBot 身份、不修改本地宗门或玩家资产；实际匹配和堡垒准入待后续切片。
