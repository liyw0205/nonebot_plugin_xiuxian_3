...
错误：`DAO_ORIGIN_REQUIREMENT_MISSING`、`TRIBULATION_TERRACE_REQUIREMENT_MISSING`、`TRIBULATION_LOCATION_REQUIRED`、`TRIAL_SEQUENCE_INVALID`、`ASCENSION_INSTANCE_BUSY`、`ASCENSION_COOLDOWN`、`ENDING_STATE_REQUIRED`、`SEASON_NOT_ACTIVE`。验收：节点顺序不可跳过；凭证锁定恢复安全；协助者人数限制；失败不双加天劫债；终局关闭不丢失候选角色结局入口。
