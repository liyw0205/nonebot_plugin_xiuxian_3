...
P4 的当前切片已经落地：感气调息与灵泉谷修炼会话、灵泉谷准入、资源锁定/取消、按快照
结算、境内/总修为、逐层晋升、30 分钟资源恢复、过期会话恢复、灵泉每日 4 次配额、感气
L1–L10 的里程碑解锁和资料卡增强。下一步按依赖顺序开放正式生产，再实现聚气突破；战斗
...
### 工程状态：P3 已验收，P4 最小切片已验收

当前目标虚拟环境的回归结果为 `25 passed`；系统 Python 为 `24 passed, 1 skipped`（缺少
OneBot V11 可选依赖）。`compileall` 与 `git diff --check` 通过。P3 写入口均使用事务和
operation ledger，展示层只显示中文阶段、境界、地点、资质和道号，不泄露平台用户 ID
或内部稳定键。P4 当前开放感气调息、灵泉谷修炼与资源恢复；正式生产、聚气以上和战斗
仍未开放。
