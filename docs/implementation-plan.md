...
5. **已完成**：固定 2v2 `arena.team`；复用已确认双人队伍，保存队伍快照、同段匹配、资产隔离、服务端行动、独立回放和 QQ/OneBot 验收。
6. **当前阶段**：实现三人以上/非对称多方 PvP 前置；通过多方快照、队伍匹配、资产隔离、服务端行动、回放恢复和适配器验收后，再扩展跨服及完整三界区域。金丹至渡劫剩余资源链继续独立验收，不得作为 PvP 夹具入口。
