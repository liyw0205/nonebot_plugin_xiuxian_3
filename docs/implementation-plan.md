...
9. **已完成（v0.2 世界垂直切片）**：云城/阵堂准入、云舟洞天二层/魔界引导航线、费用与凭证快照、`running/arrived` 恢复、operation 幂等，以及魔界风险确认和阵堂权限拒绝均已接入并通过 QQ/OneBot 模拟适配器测试；云铁采集、洞天二层战斗、阵堂生产和魔界核心区仍关闭。
10. **当前阶段**：继续独立验收完整三界区域和高阶资源链，不得作为 PvP 夹具入口，也不开放跨服匹配或身份合并。
