...
客房以 20 灵石租住 3 天，供静养闭关使用。神通参悟和低阶法器祭炼/灵纹重铸已作为独立养成切片开放；灵兽保持关闭。
随后已接入 `constitution.profile`：六选一主质、资格/道途/境界快照、体质档案查询、重塑令
消耗与 30 天冷却；道脉天书已接入当前首要道途五阶线性节点、首阶免费、天赋点扣除、
节点/点数流水和构筑快照；神通参悟已接入独立技能心得流水，低阶法器已接入装备实例与养成流水，灵兽仍保持关闭。
...
当前目标虚拟环境的回归结果为 `91 passed`；系统 Python 为 `90 passed, 1 skipped`（缺少
...
金丹以上突破、生产委托、秘境/遗府和战斗仍未开放；神通参悟与法器养成只保存构筑/装备快照，不启动战斗运行时，灵兽仍保持关闭。
