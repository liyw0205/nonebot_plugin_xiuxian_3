...
本计划以 `docs/xiuxian3-design.md`、`docs/index.md` 和[完整内容开发总表](content-development.md)为产品裁决来源。各域 `content-v*.md` 只是历史发布快照，不是并列规范。修仙3是新游戏：旧项目只能提供通用玩法分类和适配器经验，不能决定境界、数值、命令、物品、表结构或业务流程。

当前优先级：先完成文档基线与 MVP-1（凡人至筑基及基础功能闭环）的规则验收，再实施代码。工作区中已有的 P0/P1/P2 基础设施和未完成的 P3 玩家代码属于工程探索；在它们与本计划和 `content-development.md` 对齐、测试通过前，均不得作为已发布玩法。
...
P3 的首个可运行垂直切片已经完成并通过回归：`player.create`、`player.start_seeking`、
三项凡人引导、教学移动、`player.enter_cultivation`、道号规则和资料卡均由共享
application、SQLite operation ledger 与 QQ/OneBot/NoneBot 适配器接入。当前实现的首版
资源基线是寻仙后 30/30 体力、30/30 精力、粗糙灵米 ×3、止血草 ×3；入道额外发放
200 枚灵石和基础功法/技能。

P4/P5 当前切片已经落地：感气调息与灵泉谷修炼会话、灵泉谷准入、资源锁定/取消、按快照
结算、境内/总修为、逐层晋升、30 分钟资源恢复、过期会话恢复、灵泉每日 4 次配额、感气
L1–L10 里程碑解锁、资料卡，以及炼丹/炼器/布阵个人生产订单。生产已接入材料/精力/工具
耐久事务、质量快照、失败返还、每日配额和过期恢复。聚气与筑基突破已接入独立会话、失败
虚弱、保护丹、失败保底、道基质量快照和 operation 回放；金丹以上突破、生产委托和战斗
运行时仍以后置门槛为准，不在本阶段提前实现。
养成切片新增基础/静养闭关与最小居所租赁：闭关会话独占其他长时行动，按开始快照确定
修为或精力收益，支持每日配额、8 小时累计上限、24 小时过期恢复和 operation 回放；青石镇
客房以 20 灵石租住 3 天，供静养闭关使用。神通参悟已作为独立养成切片开放；法器和灵兽保持关闭。
随后已接入 `constitution.profile`：六选一主质、资格/道途/境界快照、体质档案查询、重塑令
消耗与 30 天冷却；道脉天书已接入当前首要道途五阶线性节点、首阶免费、天赋点扣除、
节点/点数流水和构筑快照；神通参悟已接入独立技能心得流水，法器和灵兽仍保持关闭。
P5 的正式世界移动已新增雾隐洞天一层入口：移动预览、2 分钟会话、体力/灵石/凭证原子扣除、
到达结算、版本快照、跨域会话互斥和 operation 回放均已接入；玄天界其他正式路线仍按后续
切片开放。探索 v0.1 已接入近郊采集、近郊短历练、灵泉采集和雾隐洞天探索：会话、每日
次数、体力扣除/取消返还、快照随机、奖励幂等和跨域互斥均已验收；遭遇战只进入
`combat_pending`。悬赏 v0.1 已接入草药补给和生产订单：当日唯一接取、背包/生产基线、
过期、领取奖励、地方名望/服务信誉和 operation 回放均已验收；训练傀儡悬赏保持
`locked`。主线道途 v0.1 已接入前三个不依赖战斗的关卡，首通唯一键、前置快照、
地点/图鉴事件、称号、声誉、重试奖励和 operation 回放均已验收；城镇委托关卡依赖
常驻经营委托，保持 `CONTENT_CLOSED`。秘境/遗府以及自动回合 PVE/PvP 运行时仍以后置切片为准。

`routine` 首片已接入道历问安、补录道历、灵木聚财、配置注入的机缘密令和道契测试凭证：签到/补录按业务日和唯一日期结算，
补录窗口与月度次数、精力封顶、连续 7 日机缘签、灵木 7 次浇灌、确定性收获池、地方名望、
24 小时冷却及 operation 回放均已验收。七日入道的起点快照、D1-D4/D7 目标投影、补做、
单日领奖和 operation 回放已接入；称号/功业录已接入来源投影、一次性领奖和装备状态；
机缘密令已接入外部配置哈希、有效期/撤销、总库存和角色唯一领取；机缘寻宝基础池已接入
单抽/十连、票券优先、十抽保底、确定性结果和 operation 回放；D5 试炼塔、D6 派遣依赖
系统仍关闭。问道行卷 v0.1 已开放周期状态、operation 点数投影、日/周上限及免费/付费领奖；战斗
运行时仍以后置门槛为准。
...
- 修复跨域内容闭合：技能命名统一为 `skill.<path>.<name>`，配方输出均有 `item.*` 定义，金丹/元婴/化神/炼虚/合道前置任务可达，v0.1 占位境界不再被当成开放内容，终局道果与 `resource.ascension_merit` 门槛具备固定可达路径。
...
### 工程状态：P3、P4 与 P5 最小切片已验收

当前目标虚拟环境的回归结果为 `84 passed`；系统 Python 为 `83 passed, 1 skipped`（缺少
OneBot V11 可选依赖）。`compileall` 与 `git diff --check` 通过。P3 写入口均使用事务和
operation ledger，展示层只显示中文阶段、境界、地点、资质和道号，不泄露平台用户 ID
或内部稳定键。P4/P5 当前开放感气调息、灵泉谷修炼、基础/静养闭关、最小居所、资源恢复、
聚气/筑基突破、首版个人生产、四种探索模式、草药/生产悬赏和当前首要道途的道脉天书；
金丹以上突破、生产委托、秘境/遗府和战斗仍未开放；神通参悟只保存构筑快照，不启动战斗运行时，法器和灵兽仍保持关闭。
