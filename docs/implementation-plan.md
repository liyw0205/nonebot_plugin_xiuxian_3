...
47. **已完成（新角色→炼虚连续验收）**：`test/test_progression_sources.py::test_qq_and_onebot_can_reach_soul_transformation_from_new_player` 在 QQ 官方和 OneBot V11 独立数据库中，从新角色沿第 42 条链继续，使用正式命令完成三界主线 5 关、3 次领域委托、67 次万兽山队伍战、3 次界隙队伍战、跨界战、化神许可、每日神魂淬炼、元婴/化神逐层晋升、公开市场交易取得灵石、三次界壁试炼、档案航道/守卫、档案交付、炼虚许可和炼虚突破。主角色未通过 SQL 写入境界、资源、资格或结果；协作者仅以受控高阶战斗属性和市场资金参与，队伍胜利、本人参与和市场结算证据均由服务端产生。专项测试两个适配器均通过，剩余缺口转为炼虚→合道→渡劫→终局完整链。
