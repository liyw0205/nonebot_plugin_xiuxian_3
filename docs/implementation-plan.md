...
本计划以 `docs/xiuxian3-design.md`、`docs/index.md` 和各域 `content-v0.1.md` 为产品裁决来源。修仙3是新游戏：旧项目只能提供通用玩法分类和适配器经验，不能决定境界、数值、命令、物品、表结构或业务流程。

当前优先级：先完成文档基线与 v0.1 新手闭环的规则验收，再实施代码。工作区中已有的 P0/P1/P2 基础设施和未完成的 P3 玩家代码属于工程探索；在它们与本计划和 `foundation/player/content-v0.1.md` 对齐、测试通过前，均不得作为已发布玩法。
...
P0 产品与内容基线冻结
  ...
  -> P5 玄天界探索、生产、单人 PVE、悬赏/秘境/主线、闭关与基础构筑
  -> P6 宗门、委托、固定市场、世界事件、机缘寻宝/行卷与经济恢复
  -> P7 魔界/妖界入口、阵营、Web 运营与隔离扩展
  -> P8 金丹以上、三界正式内容、发布周期、赛季与兼容清理
```

P7 的 Web 运营部分可并行于 P6 的稳定化；P8 只能建立在每个既有切片完成恢复演练、内容版本冻结和观测指标之后。

## 3. 分阶段交付、验收与回滚

| 阶段 | 交付物 | 主要风险和控制 | 最低验收 | 回滚点 |
|:--|:--|:--|:--|:--|
| P0 | 设计总纲、域索引、v0.1-v0.6 内容基线、稳定键、数值和参考边界 | 旧玩法/数值回流；文档交叉链接与稳定键检查 | `python3 scripts/validate_content_docs.py`、`git diff --check` | 仅撤回未发布的文档变更；不改运行数据 |
...
| P5 | 玄天界移动、短历练、雾隐洞天、基础生产、单人 PVE、筑基准备 | 会话并发、战斗重复结算、配方通胀 | 地点条件、会话状态机、战斗快照、生产失败、探索过期 | 停止新会话，结算/取消已有会话，恢复世界快照 |
| P6 | 宗门、师徒、队伍、委托、固定摊位、任务、灵泉事件 | 跨玩家锁定、市场悬挂、奖励重复 | 锁定/成交/过期恢复、贡献和领奖幂等、事件轮次回放 | 冻结交易与领奖，执行补偿/回收，恢复市场与事件快照 |
...
- v0.1 的 `player-onboarding-v0.1.0`、`qualification.v0.1`、`gather.v0.1`、`progression-0.1.1` 和对应内容键是固定基线；十层阈值以 `foundation/progression/layers.md` 为唯一权威。
...
当前文档阶段不继续增加玩法代码。已有未完成的 P3 `Player`/注册探索代码不得按其临时“凡人即已注册、100 体力、直接注册成功”行为合入，因为它与 v0.1 内容基线不一致；后续以本计划重写/迁移为 `new_user -> 寻仙问道 -> mortal` 的状态机。

文档验收通过后，下一步是 P3.1：为 `player.create` 编写失败测试、实现最小身份映射和只读资料，再逐个垂直切片进入寻仙问道与引导。
...
- 为全部 121 份 `content-v0.1.md` 至 `content-v0.6.md` 补齐版本、稳定键、前置、成本、产出、随机/快照、失败、幂等、权限、观测、关闭和回滚合同；新增 `scripts/validate_content_docs.py` 作为可重复的内容校验器。
...
- `python3 scripts/validate_content_docs.py`：121 份版本内容文件通过合同、链接、发布边界、关键键、任务、终局可达性、十层、无娱乐域、常驻经营、特色系统和上游隔离校验。
- 本地 Markdown 链接检查：192 份文档全部通过。
- `git diff --check`：通过。

### 工程状态：未验收，已冻结

本轮没有把既有探索性 P3 代码视为完成玩法。当前 `.venv/bin/python -m pytest -q` 结果为
`44 passed, 6 subtests passed, 3 failed`：一项 operation 冲突尚未转换为 application
`Result`，两项迁移/备份测试仍将 schema version 硬编码为 1；更重要的是现有注册代码
不符合本计划的 `new_user -> 寻仙问道 -> mortal` 产品状态机。后续 P3.1 必须从本内容基线
重新做 test-first 垂直切片，而不是继续补丁式扩展该临时实现。