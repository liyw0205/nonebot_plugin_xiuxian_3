...
12. **当前阶段（v0.3 魔界单人切片，`partial`）**：魔界堕落遗迹单人探索已接入元婴 L1/每日次数/污染上限/跨界惩罚/魔核或魔界声望掉落/失败神魂疲劳，探索与自动战斗均冻结 `content-0.3` 及对应规则版本；`quest.demon_main_1` 通过已结算探索会话核验两次胜利和魔界声望 200，原子写入 `quest_events`、`quest_progress`、`intro_json` 与 operation；`event.demon_invasion` 已接入每周三 20:00 UTC 的 3 小时轮次、运输/个人设施维护/战斗来源投影、个人贡献 50 门槛和双适配器领奖，个人设施可由玩家按服务端业务日维护，`xuantian.war_front` 可在活动窗口内移动并开始固定自动战。正式契约线索来源、魔界完整核心区、妖界万兽山和跨界 2–5 人副本仍未开放。
