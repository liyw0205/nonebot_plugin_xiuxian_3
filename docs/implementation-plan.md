...
34. **已完成（v0.1 上游材料补齐）**：按既有生产内容合同注册 `recipe.pill.focus_low`，感气炼丹辅修角色可用灵叶 2、止血草 1、基础丹炉和 5 精力制作焦点丹；订单日限、失败返还、工具耐久、operation 重放和 QQ/OneBot 命令路径已验收。筑基丹的正式来源数值尚未在内容合同中确定，仍不以夹具冒充玩家路径。
