...
26. **已完成（v0.5 虚空档案阶段 A）**：新增独立 `void_archive` 规则、模型、repository、用例和迁移；`void.archive_ruins` 已结算航道绑定 `enemy.archive_keeper` 服务端自动战，同一航道唯一结算，档案产出每周 1 份，超限转虚空晶；新增 `recipe.void.crystal_refine`，并以航道结算、守卫胜利、生产完成订单投影 alpha/beta/gamma 周任务。三项任务按 UTC 周唯一领奖，完成后激活 7 日 `event.archive_unlock`；QQ 官方与 OneBot V11 共享 application 和 operation ledger 已覆盖。下一项为 `season.void_frontier` 赛季积分、冻结、周任务奖励箱和赛季奖励；跨服匹配、身份合并、玩家资产转移仍关闭。
