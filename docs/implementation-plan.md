...
35. **已完成（v0.1 突破保护来源）**：按 `production-0.1.1` 注册 `recipe.pill.qi_guard` 与 `recipe.pill.foundation_guard`；感气/聚气炼丹辅修角色可分别制作聚气护脉丹、筑基护脉丹，绑定成品只能在对应突破失败时消耗，成功不消耗。两条配方的材料、精力、工具耐久、日限、失败返还、不可交易边界和 QQ/OneBot 命令路径已验收；筑基护脉丹宗门商店入口仍待固定兑换切片。
