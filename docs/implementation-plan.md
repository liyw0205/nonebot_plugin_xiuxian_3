...
37. **进行中（v0.2→v0.4 晋升上游）**：修复凝核丹只能由金丹角色生产却是金丹突破必需材料的循环依赖；筑基炼丹辅修可通过个人生产获得绑定凝核丹，金丹生产保持开放。筑基可由青石镇往返矿区，接取有效期内的云铁悬赏取得采集权限；QQ/OneBot 已从受控筑基资源起点通过正式移动→委托→采集云铁→生产→金丹突破链；感气/聚气角色仍被拒绝。下一步仍需从新角色无数据库夹具地串通筑基、金丹、元婴、化神及后续境界。
