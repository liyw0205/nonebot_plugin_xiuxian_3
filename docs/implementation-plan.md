...
P0 产品与内容基线冻结
  ...
  -> 自动回合 PVE 扩展切片：训练/具名遭遇/天劫（已部分开放，继续做回放与数值验收）
  -> 多人 PVE / 终局战 / PvP（必须分别通过前置门槛）
...
训练傀儡、具名高阶遭遇和天劫试炼已经通过最小自动回合切片接入。新增探索战、多人 PVE、
...
1. 完成天劫三阶段自动回合的高阶属性、技能机制、回放恢复和失败/成功结算验收。
2. 补齐探索遭遇和多人 PVE；多人会话必须使用队伍快照、锁定资产和唯一奖励结算。
3. 建立终局最终战多人会话（1 名发起者、最多 4 名协助者），再开放飞升/留界真实玩家路径。
4. 最后实现终局天榜、赛季候选/三榜和正式 1v1/多方 PvP；金丹至渡劫内容继续按独立境界切片开放。
