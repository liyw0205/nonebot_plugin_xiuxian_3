...
12. **当前阶段（v0.3 魔界单人切片，`partial`）**：魔界堕落遗迹单人探索已接入元婴 L1/每日次数/污染上限/跨界惩罚/魔核或魔界声望掉落/失败神魂疲劳，探索与自动战斗均冻结 `content-0.3` 及对应规则版本；`quest.demon_main_1` 通过已结算探索会话核验两次胜利和魔界声望 200，原子写入 `quest_events`、`quest_progress`、`intro_json` 与 operation，并通过 QQ 官方和 OneBot V11 模拟适配器验收。正式 `event.demon_invasion`、契约线索来源、魔界完整核心区、妖界万兽山和跨界 2–5 人副本仍未开放。
