...
1. `routine`：道历问安/补录、灵木聚财、七日入道、道号/功业、机缘密令。
...
机缘密令已接入外部配置哈希、有效期/撤销、总库存和角色唯一领取；D5 试炼塔、D6 派遣依赖
系统仍关闭。机缘寻宝、问道行卷仍未开放；战斗
运行时仍以后置门槛为准。
...
当前目标虚拟环境的回归结果为 `59 passed`；系统 Python 为 `58 passed, 1 skipped`（缺少
