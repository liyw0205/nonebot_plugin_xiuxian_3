...
| 问题 | 唯一入口 | 不要从哪里推断 |
|:--|:--|:--|
| 产品范围、境界、稳定键和完整路线 | [完整内容开发总表](content-development.md) | 不从历史快照或命令处理器推断 |
| 当前分支是否已可运行 | [当前开发状态](current-status.md) | 不从 `content-v0.*` 的 `open` 推断 |
| 下一条垂直切片如何拆分和验收 | [实施计划](implementation-plan.md) | 不按 v0.1 到 v0.6 机械排期 |
| 某个域的实体、状态机和用例 | 对应域的 `README.md`、`model.md`、`workflow.md`、`use-cases.md` | 不复制到适配器或命令层 |
| 历史发布参数 | 对应域的 `content-v0.*.md` | 不把历史参数当作当前运行时状态 |
...
当前运行基线：`data/内容清单.json` 是静态内容包入口，负责注册稳定键和版本；它的
`active/open` 只表示配置已加载，不等于玩家入口已经开放。玩家可用能力、关闭新建和
下一条切片统一以[当前开发状态](current-status.md)为准。天劫三阶段自动回合已经完成
快照、技能、债务护盾、回放恢复和结算验收；探索遭遇和双人队伍 PVE 已接入自动回合、成员快照与唯一奖励结算，下一条执行切片是终局多人 PVE，终局
最终战、多人协助、飞升/留界真实路径和 PvP 仍保持关闭。
