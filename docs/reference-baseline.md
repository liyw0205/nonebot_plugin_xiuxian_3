...
存在，只确认它的 remote、分支、HEAD 和 `git status`，不满足 `main` 干净条件
就停止并报告，不要自动删除或重新克隆。除非明确要求刷新基线，否则后续直接
复用这个目录。远端还存在其它分支，但本仓库不以它们为基线。
...
参考副本应长期保留并复用；只有明确要求刷新基线时才重新克隆。新的盘点应在
本文件增加提交号、目录校验和变更摘要，而不是静默覆盖旧基线。任何 goal 的
提示词都必须携带这条隔离要求。
