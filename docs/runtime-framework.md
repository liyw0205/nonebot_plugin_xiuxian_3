...
闭关修行使用独立的长时会话，不与普通修炼、生产、移动、探索或突破并行：
...
基础闭关要求入道、基础功法和粗糙灵米，持续 2 小时，消耗 4 点精力和 1 份灵米，
每日最多 3 次，结算获得确定性 80/100/120 点境内修为。静养闭关要求有效居所，
持续 4 小时，消耗 2 点精力，每日 1 次，恢复最多 8 点精力且不增加修为。当前开放的
青石镇客房租期 3 天、租金 20 灵石；居所与闭关均保存开始时的规则/资源快照。
结算最多累计 8 小时，超过 24 小时标记为过期，必须使用 `恢复闭关`；重复 operation
只回放原结果。闭关期间所有互斥入口在同一事务内拒绝，避免并发创建两个长时会话。
...
整数/bp 写入档案，后续战斗、生产或探索结算读取开始时快照；当前已开放的自动回合 PVE
（包括探索遭遇和双人队伍 PVE）读取该快照，三人以上战斗和 PvP 仍按当前状态表关闭。
...
首阶免费，后续依次消耗 1/2/3/5 点角色绑定天赋点；点数扣除、节点状态和快照在同一
事务中记录，`talent.reset_tree` 因重置规则未冻结暂不开放。
...
基础攻击和当前首要道途主动技能可参悟至 3 级，按目标等级消耗技能心得/灵石
`1/20`、`1/40`、`2/80`；每级效果增加 300 bp。参悟、资源扣除、技能快照、技能心得流水
和 operation ledger 在同一事务中完成，当前不会启动战斗或开放手动操作。

低阶法器养成使用独立装备实例，不把强化等级或词条写回堆叠背包：
...
木纹剑与棉袍最高 3 阶，祭炼失败不降级；灵纹重铸失败保留旧词条，连续失败 3 次后下一次
保底获得非空词条。旧版本库存中的唯一法器会在同一祭炼/重铸事务中懒迁移为装备实例，资源、
实例状态、快照、流水和 operation ledger 原子写入。法器养成与修炼、生产、探索、移动、闭关、
突破等长时行动互斥，且入口继续统一经过 `XiuxianApplication._invoke` 的身份校验。

感气二层并完成教学采集后，教学快捷入口可发送 `前往灵泉谷`；要走正式可结算的世界移动，
从新手城或近郊发送 `前往 灵泉谷`，等待 90 秒后发送 `结算移动`，消耗 4 点体力抵达灵泉谷。
在灵泉谷发送 `开始修炼 灵泉` 可创建 15 分钟的灵泉修炼，消耗 3 点体力，基础修为 70，使用
...
此状态由已登记的战斗域入口自动推进，战斗胜利后才发放冻结探索奖励，失败不发奖励。历史
挂起会话支持重启恢复；三人以上自动回合 PVE、终局最终战和 PvP 仍等待对应前置门槛。
...
训练傀儡悬赏只展示为锁定；战斗域的训练会话与悬赏域隔离，悬赏入口不会创建战斗会话或悬赏会话。
