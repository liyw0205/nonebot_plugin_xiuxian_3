...
整数/bp 写入档案，后续战斗、生产或探索结算读取开始时快照；当前仅已开放的自动回合 PVE
读取该快照，探索遭遇、多人战斗和 PvP 仍按当前状态表关闭。
...
此状态只由已登记的战斗域入口继续推进；未登记的探索遭遇仍稳定返回“战斗待处理”，不会
抽取探索奖励或自行启动战斗。自动回合多人 PVE、终局最终战和 PvP 仍等待对应前置门槛。
