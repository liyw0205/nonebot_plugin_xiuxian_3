...
def register_core_commands(router: CommandRouter, application: XiuxianApplication) -> None:
    ...
    router.register("登上云舟", application.board_cloud_boat, aliases=("乘坐云舟", "云舟航线"))
    ...
    router.register("三界主线", application.get_three_realms_status)
    router.register("开始三界主线", application.start_three_realms_stage)
    router.register("领取三界主线奖励", application.claim_three_realms_stage)
    ...
    router.register("宗门商店", application.get_sect_shop)
    router.register("宗门商店兑换", application.exchange_sect_item, aliases=("宗门兑换",))
    router.register("宗门每日建设", application.build_sect_daily)
    router.register("宗门捐献", application.donate_sect_asset)
    router.register("宗门补给", application.procure_sect_stock)
    router.register("宗门战", application.get_sect_war)
    router.register("报名宗门战", application.register_sect_war)
    router.register("贡献宗门战", application.contribute_sect_war)
    router.register("领取宗门战奖励", application.claim_sect_war_reward)
    router.register("虚空堡垒", application.get_void_fortress, aliases=("堡垒状态",))
    router.register("建造虚空堡垒", application.build_void_fortress)
    router.register("维护虚空堡垒", application.maintain_void_fortress)
    router.register("跨服宗门战", application.get_cross_server_sect_war)
    router.register("报名跨服宗门战", application.register_cross_server_sect_war)
    router.register("跨服宗门战分支", application.choose_cross_server_sect_war_branch)
    router.register("贡献跨服宗门战", application.contribute_cross_server_sect_war)
    router.register("领取跨服宗门战奖励", application.claim_cross_server_sect_war_reward)
    router.register("分配跨服宗门战奖励", application.allocate_cross_server_sect_war_reward)
    router.register("虚空前线", application.get_void_frontier_season, aliases=("虚空前线赛季", "虚空前线状态"))
    router.register("领取虚空前线周任务", application.claim_void_frontier_weekly, aliases=("领取虚空前线周奖励",))
    router.register("领取虚空前线奖励", application.claim_void_frontier_reward, aliases=("领取虚空前线赛季奖励",))
    router.register("虚空信标", application.get_void_beacon, aliases=("信标状态",))
    router.register("建造虚空信标", application.build_void_beacon)
    router.register("维护虚空信标", application.maintain_void_beacon)
    router.register("生产联盟", application.get_sect_alliance)
    router.register("发起生产联盟", application.create_sect_alliance)
    router.register("确认生产联盟", application.confirm_sect_alliance)
    router.register("解除生产联盟", application.end_sect_alliance)
    router.register("同步联盟配方", application.sync_sect_alliance_research)
    router.register("创建跨服社交备份", application.create_social_recovery_backup)
    router.register("校验跨服社交备份", application.verify_social_recovery_backup)
    router.register("恢复跨服社交备份", application.restore_social_recovery_backup)
    ...
    router.register("创建魔渊队伍", application.create_demon_party, aliases=("创建魔界副本队伍",))
    router.register("创建万兽队伍", application.create_beast_party, aliases=("创建妖界副本队伍",))
    router.register("创建多人副本队伍", application.create_standard_pve_party, aliases=("创建四人副本队伍", "创建普通副本队伍"))
    ...
    router.register("发布求购", application.create_purchase_order, aliases=("发布跨界求购",))
    router.register("求购列表", application.list_purchase_orders, aliases=("跨界求购列表", "查看求购"))
    router.register("匹配求购", application.match_purchase_order, aliases=("接取求购", "承接求购"))
    router.register("交付求购", application.deliver_purchase_order, aliases=("完成求购", "卖出求购"))
    router.register("取消求购", application.cancel_purchase_order)
    router.register("清理求购", application.expire_purchase_order, aliases=("恢复求购",))
    ...
    router.register("发布拍卖", application.create_auction)
    router.register("拍卖列表", application.list_auctions, aliases=("查看拍卖",))
    router.register("竞价", application.bid_auction)
    router.register("结算拍卖", application.settle_auction)
    ...
    router.register("领取魔界入侵奖励", application.claim_demon_invasion_event)
    router.register("妖界贸易事件", application.get_beast_trade_event, aliases=("万兽贸易事件",))
    router.register("贡献妖界贸易", application.contribute_beast_trade_event)
    router.register("领取妖界贸易奖励", application.claim_beast_trade_event)
    router.register("界隙裂痕", application.get_boundary_rift_event, aliases=("界隙事件",))
    router.register("贡献界隙裂痕", application.contribute_boundary_rift_event)
    router.register("领取界隙裂痕奖励", application.claim_boundary_rift_event)
    router.register("领域前线", application.get_domain_front, aliases=("领域前线状态",))
    router.register("加入领域前线", application.join_domain_front)
    router.register("开始领域战", application.start_domain_front_battle, aliases=("领域战",))
    router.register("占点领域前线", application.create_domain_front_point, aliases=("领域前线占点",))
    router.register("贡献领域前线", application.contribute_domain_front)
    router.register("领取领域前线奖励", application.claim_domain_front_reward)
    ...
    router.register("领域赛季", application.get_domain_war_season)
    router.register("领取领域赛季奖励", application.claim_domain_war_reward)
    router.register("兑换领域核心", application.redeem_domain_core)
    router.register("发布竞技场快照", application.publish_arena_snapshot, aliases=("发布防守快照",))
    router.register("发布三界竞技场快照", application.publish_three_realms_arena_snapshot, aliases=("发布三界防守快照",))
    ...
    router.register("三界竞技场列表", application.list_three_realms_arena_snapshots, aliases=("三界竞技场匹配池",))
    router.register("挑战竞技场", application.challenge_arena, aliases=("竞技场挑战",))
    router.register("挑战三界竞技场", application.challenge_three_realms_arena, aliases=("三界竞技场挑战",))
    ...
    router.register("发布组队竞技场快照", application.publish_team_arena_snapshot, aliases=("发布2v2快照",))
    router.register("组队竞技场列表", application.list_team_arena_snapshots, aliases=("2v2匹配池",))
    router.register("挑战组队竞技场", application.challenge_team_arena, aliases=("组队竞技场挑战",))
    router.register("组队竞技场回放", application.replay_team_arena, aliases=("回放组队竞技场",))
