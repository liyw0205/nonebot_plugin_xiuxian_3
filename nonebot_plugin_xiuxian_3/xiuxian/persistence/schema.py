...
CREATE TABLE IF NOT EXISTS arena_recovery_events (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    event_key TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_arena_recovery_events_operation
    ON arena_recovery_events(operation_id, created_at);
...
CREATE TABLE IF NOT EXISTS arena_team_snapshots (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    snapshot_id TEXT NOT NULL UNIQUE,
    party_id TEXT NOT NULL REFERENCES parties(party_id),
    leader_id INTEGER NOT NULL REFERENCES players(id),
...
CREATE INDEX IF NOT EXISTS idx_arena_team_snapshots_pool
    ON arena_team_snapshots(status, rating, matchable_at, expires_at);
CREATE INDEX IF NOT EXISTS idx_arena_team_snapshots_party
    ON arena_team_snapshots(party_id, status, created_at);
CREATE UNIQUE INDEX IF NOT EXISTS idx_arena_team_snapshots_active_party
    ON arena_team_snapshots(party_id) WHERE status = 'published';

CREATE TABLE IF NOT EXISTS arena_team_matches (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    match_id TEXT NOT NULL UNIQUE,
    challenger_leader_id INTEGER NOT NULL REFERENCES players(id),
    defender_leader_id INTEGER NOT NULL REFERENCES players(id),
    challenger_snapshot_id TEXT NOT NULL REFERENCES arena_team_snapshots(snapshot_id),
    defender_snapshot_id TEXT NOT NULL REFERENCES arena_team_snapshots(snapshot_id),
    status TEXT NOT NULL CHECK (status IN ('settled')),
    outcome TEXT NOT NULL CHECK (outcome IN ('challenger_won', 'defender_won', 'draw')),
    rounds INTEGER NOT NULL CHECK (rounds BETWEEN 1 AND 15),
    challenger_rating_delta INTEGER NOT NULL,
    ...
    created_at TEXT NOT NULL,
    settled_at TEXT NOT NULL,
    CHECK (challenger_leader_id <> defender_leader_id)
);

CREATE INDEX IF NOT EXISTS idx_arena_team_matches_challenger
    ON arena_team_matches(challenger_leader_id, created_at);
CREATE INDEX IF NOT EXISTS idx_arena_team_matches_defender_snapshot
    ON arena_team_matches(defender_leader_id, defender_snapshot_id, created_at);

CREATE TABLE IF NOT EXISTS arena_team_actions (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    action_id TEXT NOT NULL UNIQUE,
    match_id TEXT NOT NULL REFERENCES arena_team_matches(match_id),
    sequence_no INTEGER NOT NULL CHECK (sequence_no >= 1),
    round_no INTEGER NOT NULL CHECK (round_no BETWEEN 1 AND 15),
    actor_key TEXT NOT NULL,
    strategy_key TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_arena_team_actions_match
    ON arena_team_actions(match_id, sequence_no);
