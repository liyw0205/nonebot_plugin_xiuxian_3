        ...
        for adapter in ("qq.official", "onebot.v11"):
            with TemporaryDirectory() as directory:
                ...
                with sqlite3.connect(runtime.settings.database_path) as connection:
                    connection.execute(
                        "UPDATE players SET realm_key='foundation', realm_layer=10, cultivation=7700, total_cultivation=11960, foundation_quality=4000, energy=30, spirit_stones=2000, inventory_json=? WHERE platform=? AND platform_user_id=?",
                        (json.dumps({
                            "item.herb.spirit_leaf": 5, "item.material.cloud_iron": 5,
